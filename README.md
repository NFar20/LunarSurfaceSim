...
This information, along with the material properties, is used to write ODEs, which are then solved to determine the temperature at each given time.
V1 treats the object as a point mass, whereas V2 treats each surface and the center of the cube as separate nodes and individually calculates each surface's temperature.
...
When installing, make sure the "data" and "kernels" folders are in the same directory as "LunarSimV#.py" and that all required Python libraries are installed.
The required libraries can be seen at the beginning of each Python file.
